...
const App = () => {
    ...
    const fetchMovies = async () => {
        const response = await fetch('https://hoomans.onrender.com/api/movies');
    ...
    const handleSearch = async () => {
        setIsSearching(true);
        const response = await fetch(`https://hoomans.onrender.com/api/movies/search?title=${searchQuery}`);
        const data = await response.json();
        setMovies(data);
        setIsMainPage(false);
    };
